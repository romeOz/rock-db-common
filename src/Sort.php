 ...
 * ```
 *
 * In the above, we declare two {@see \rock\db\common\Sort::$attributes} that support sorting: name and age.
...
class Sort implements ObjectInterface
{
     ...
     * - The "label" element specifies what label should be used when calling {@see \rock\db\common\Sort::link()} to create
     ...
     *
     * The array element indexed by {@see \rock\db\common\Sort::$sortParam} is considered to be the current sort directions.
     * If the element does not exist, the {@see \rock\db\common\Sort::$defaultOrder} default order will be used.
     *
     * @see sortParam
     * @see defaultOrder
     */
    ...
    /**
     * Normalizes the {@see \rock\db\common\Sort::$attributes} property.
     */
    ...
    /**
     * @var array the currently requested sort order as computed by {@see \rock\db\common\Sort::getAttributeOrders()} .
     */
     ...
     * If this is not set, the label defined in {@see \rock\db\common\Sort::$attributes} will be used.
     * If no label is defined, {@see \rock\helpers\Inflector::camel2words()} will be called to get a label.
     * Note that it will not be HTML-encoded.
     * @return string the generated hyperlink
     * @throws DbException if the attribute is unknown
     */
     ...
     * This method will consider the current sorting status given by {@see \rock\db\common\Sort::$attributeOrders}.
     ...
     * @throws DbException if the specified attribute is not defined in {@see \rock\db\common\Sort::$attributes}
     */
    public function createSortParam($attribute)
    {
        if (!isset($this->attributes[$attribute])) {
            throw new DbException("Unknown attribute: {$attribute}");
        }
        $definition = $this->attributes[$attribute];
    ...
    protected function getParam()
    {
        if (empty($this->params)) {
            if ($this->request instanceof Request) {
                return Request::get($this->sortParam);
            }
